// =============================================================================
// <ultraviolet/utf/utf8/utf16/sse.c>
//
...
simd:
  while (i <= p)
  {
    ...
    if (likely (ao == 0))
    {
      ...
      sz += 16u;
#else
      _mm_storeu_si128 ((xi128*)(void*)o, _mm_unpacklo_epi8 (xi, xzero));
      _mm_storeu_si128 ((xi128*)(void*)(o + 8u), _mm_unpackhi_epi8 (xi, xzero));
    ...
    if (unlikely (_mm_movemask_epi8 (x3) == 0))
    {
      ...
      _mm_storeu_si128 ((xi128*)(void*)(o + 8u), xuh);
    ...
    | _mm_cmpestrc (xrsrv, 4, xul, 8, _SIDD_UWORD_OPS | _SIDD_CMP_EQUAL_ANY
      ...
      #else
    // `pcmpistri` is faster than `pcmpestri`
