...
simd:
  while (i <= p)
  {
    ...
    if (likely (ao == 0))
    {
      ...
      _mm_storeu_si128 ((xi128*)(o + 8u), _mm_unpackhi_epi8 (xi, xz));
    ...
    if (unlikely (_mm_movemask_epi8 (x3) == 0))
    {
      ...
      _mm_storeu_si128 ((xi128*)(o + 8u), xuh);
    ...
    // Check for Unicode non-characters and reserved Unicode characters
      ...
      #else
        // `pcmpestri` is generally slow. When parsing from null-terminated strings
        // it is possible to use `pcmpistri`, which can be noticeably faster.
