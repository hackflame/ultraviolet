...

{
...
#endif
  {
    ...
    // Two UTF-8 bytes
    else if (likely (utf8_byte_is_lead2 (c)))
    {
#if T(EXPLICIT)
      ...
      u16 w = utf8_chr_make2 (c, c1);

#if T(VALID)
      // Check for overlong sequence
      if (unlikely (utf16_chr_is_lead1 (w))) goto invalid;
#endif
    ...
    else if (likely (utf8_byte_is_lead3 (c)))
    {
      ...
      u16 w = utf8_chr_set_lead3 (c) | ((cv & 0x3Fu) << 6) | ((cv & 0x3F00u) >> 8);
  #else
      u16 w = utf8_chr_set_lead3 (c) | ((cv & 0x3F00u) >> 2) | (cv & 0x3Fu);
  #endif
...
#if T(VALID)
      // Check for overlong sequence
      if (unlikely (utf16_chr_is_lead2 (w))) goto invalid;
...
#endif
    {
      ...
      u32 w = utf8_chr_set_lead4 (c) | ((cv & 0x3F00u) << 6) | ((cv & 0x3Fu) << 12) | ((cv & 0x3F0000u) >> 16);
  #else
    #if T(VALID)
      if (unlikely ((cv & 0xC0C0C0u) != 0x808080u)) goto invalid;
    #endif

      u32 w = utf8_chr_set_lead4 (c) | ((cv & 0x3F0000u) >> 4) | ((cv & 0x3F00u) >> 2) | (cv & 0x3Fu);
  #endif
      ...
      u32 w = utf8_chr_make4 (c, c1, c2, c3);
#else
      uint c1 = i[1];
      ...
      o[0] = utf16_make_surr_high (w);
      o[1] = utf16_make_surr_low (w);
