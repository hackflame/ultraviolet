...
// <utf8/utf8to16.c>
//
// UTF-8 to UTF-16 conversion template.
//
...
recover:
  while (i <= p)
  {
    ...
    if (likely (ao == 0))
    {
      ...
      sz += 16u;
  #else
      _mm_storeu_si128 ((xi128*)o, _mm_unpacklo_epi8 (xi, xz));
      _mm_storeu_si128 ((xi128*)(o + 8), _mm_unpackhi_epi8 (xi, xz));
    ...
    if (unlikely (_mm_movemask_epi8 (x3) == 0))
    {
      ...
      // Split the vector in low and high halves and compute the UTF-16 character
      ...
      // Produce the final UTF-16 characters
      ...
      // Store the UTF-16 converted vectors and advance the pointers
      _mm_storeu_si128 ((xi128*)o, xul);
      _mm_storeu_si128 ((xi128*)(o + 8), xuh);
    ...
    // due to need of surrogate pairs composition
...
needspace:
    ...
    // Two UTF-8 bytes
    else if (likely (utf8_chr_is_head2 (c)))
    {
#if T(EXPLICIT)
...
invalid:
      ...
      u16 w = utf8_codep_comp2 (c, c1);

#if T(VALID)
      // Check for overlong sequence
      if (unlikely (utf16_chr_is8_lead1 (w))) goto invalid;
#endif
    ...
    else if (likely (utf8_chr_is_head3 (c)))
    {
      ...
      u16 w = utf8_codep_comp_head3 (c) | ((cv & 0x3Fu) << 6) | ((cv & 0x3F00u) >> 8);
  #else
      u16 w = utf8_codep_comp_head3 (c) | ((cv & 0x3F00u) >> 2) | (cv & 0x3Fu);
  #endif
      ...
      u16 w = utf8_codep_comp3 (c, c1, c2);
#else
      uint c1 = i[1];
...
#if T(VALID)
      // Check for overlong sequence
      if (unlikely (utf16_chr_is8_lead2 (w))) goto invalid;
...
#endif
    {
      ...
      u32 w = utf8_codep_comp_head4 (c) | ((cv & 0x3F00u) << 6) | ((cv & 0x3Fu) << 12) | ((cv & 0x3F0000u) >> 16);
  #else
    #if T(VALID)
      if (unlikely ((cv & 0xC0C0C0u) != 0x808080u)) goto invalid;
    #endif

      u32 w = utf8_codep_comp_head4 (c) | ((cv & 0x3F0000u) >> 4) | ((cv & 0x3F00u) >> 2) | (cv & 0x3Fu);
  #endif
      ...
      u32 w = utf8_codep_comp4 (c, c1, c2, c3);
#else
      uint c1 = i[1];
